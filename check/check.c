...
void AskCopyObject (POS pos1, POS pos2)
   ...
   c = UserKey (" Copy to which FN key ? (F1 - F10 or <Esc> ");
   if (c >= K_F1 && c <= K_F10)
...
BOOL Repeating (void)
   ...
   pc1 = ppcBuffers [(iBUFIDX + 0) % SCREEN_BUFFERS];
...
void AutoFind (BOOL bQuietly)
   ...
   while (TRUE)
      ...
      bRepeating = FALSE;
      for (i=0; !bRepeating && i<MAX_AUTO_STEPS; i++)
         {
         GenStep (bQuietly);
         ...
         if (!bQuietly || !(i % 10))
            {
            sprintf (sz, "%4.4d", i);
            ScrWriteStrAtt (sz, 4, YSIZ-1, XSIZ-4, ATT_THINGIE);
            }
         if (!bQuietly)
...
void EditMode (void)
   ...
   while (!bBreak)
      ...
      switch (c)
         ...
         case K_RET  : Set (iX, iY, 1);             break;
         case ' '    : Set (iX, iY, 2);             break;
         case 'G'    : bBreak = TRUE;               break;
         case 'H'    : Commands ();                 break;
         case 'L'    : LoadObject (posHere);        break;
         ...
         case '-':
            iBUFIDX = (iBUFIDX+SCREEN_BUFFERS-1) % SCREEN_BUFFERS;
            pcScr = ppcBuffers [iBUFIDX];
            Display (-1, 0, 0, 0);
            break;
...
INT _cdecl main (INT argc, char *argv[])
   ...
   PasteBlock (argv[1], 30, 20);
   for (i=0; i<MAX_AUTO_STEPS; i++
